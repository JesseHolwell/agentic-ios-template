...
## Agent workflow (personal profile)

**Profile:** personal — see `~/Source/agent-config/profiles/personal.md`. The
agent commits, pushes and merges `main` itself once the gate passes; no pull
request is involved. The gate is the fast suite green plus the app actually run
in the simulator on the path that changed.
